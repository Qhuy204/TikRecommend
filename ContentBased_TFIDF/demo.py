#!/usr/bin/env python3
"""
Demo Module for TF-IDF Content-Based Recommender
...
class DemoRecommender:
    """
    TF-IDF Content-Based Demo - Load once, use many times
    ...
    def __init__(self):
        """Initialize - load data and model once"""
        print("=" * 50)
        print("TF-IDF Content-Based Demo")
        print("=" * 50)
        
        # Load data and TF-IDF matrix
        print("\nLoading data...")
        self.data, tfidf_matrix = load_processed_data()
    ...
    def recommend_user(self, user_id: int = None, top_k: int = 10) -> List[Dict]:
        ...
        for item_id in liked_item_ids[:5]:  # Show first 5
    ...
    def similar_items(self, item_id: int = None, top_k: int = 10) -> List[Dict]:
        ...
        Args:
            item_id: Query product ID (random if None)
    ...
    def recommend_sequence(self, item_ids: List[int], top_k: int = 10) -> List[Dict]:
        """
        Recommend based on a list of items (for cold-start/anonymous users)
